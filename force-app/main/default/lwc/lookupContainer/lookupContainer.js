...
export default class LookupContainer extends LightningElement 
{
    ...
    // EXPOSED FUNCTIONS
    @api get setsearchresults() {
        return this.setsearchresults;
    }

    set setsearchresults(results) {
        if (results) {
    ...
    selectionValueEvt() {
        const selected = new CustomEvent('selectionchange', {
    ...
    get inputColumnClass(){
        let css = 'slds-size_12-of-12';
        if (this.isMultiEntry) {
            css = 'slds-size_8-of-12';
        }
        return css;
    }
